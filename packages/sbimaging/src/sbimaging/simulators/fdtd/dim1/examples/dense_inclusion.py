"""Example: 1D acoustic simulation with a denser, wider inclusion.
...
- A wider inclusion shifted to the left with higher density and wave speed (3x)
...
def main():
    ...
    logger.info("1D Linear Acoustics FDTD - Dense Inclusion Example")
    logger.info("=" * 60)
    ...
    # Wider inclusion shifted to the left with higher density/wavespeed
    inclusion_center = 0.35  # Shifted left from 0.5
    inclusion_width = 0.15   # Wider than 0.1
    inclusion_x_min = inclusion_center - inclusion_width / 2
    inclusion_x_max = inclusion_center + inclusion_width / 2
    inclusion_density = 3.0  # Higher than 2.0
    inclusion_speed = 3.0    # Higher than 2.0
    ...
    video_file = output_dir / "1d_dense_inclusion.mp4"
    ...
    success = create_video_with_sensors(
        ...
        inclusion_alpha=0.5,  # More opaque to show higher density
        vmin=-1.0,
        vmax=1.0,
        fps=15,
        dpi=150,
    ...
    # Save sensor data
    sensor_file = output_dir / "1d_dense_sensor_data.npz"
