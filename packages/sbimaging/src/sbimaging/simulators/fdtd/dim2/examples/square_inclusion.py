"""Example: 2D acoustic simulation with a pressure source and square inclusion.

This example demonstrates a simple 2D linear acoustics FDTD simulation
with:
- A Gaussian pulse pressure source
- A square inclusion with different material properties
...
def main():
    ...
    logger.info("2D Linear Acoustics FDTD - Square Inclusion Example")
    logger.info("=" * 60)
    ...
    # Square inclusion, off-center (lower left quadrant)
    # Square with similar area to a circle of radius 0.1 (area ~ 0.031)
    # Square with side ~ 0.18 has area ~ 0.032
    square_side = 0.18
    square_center = (0.35, 0.4)  # Lower left of center
    square_x_min = square_center[0] - square_side / 2
    square_y_min = square_center[1] - square_side / 2
    square_x_max = square_center[0] + square_side / 2
    square_y_max = square_center[1] + square_side / 2
    ...
    logger.info(
        f"Square inclusion at center ({square_center[0]}, {square_center[1]}), "
        f"side={square_side}m, "
    ...
    video_file = output_dir / "square_inclusion.mp4"

    inclusion_overlay = RectangleOverlay(
        x_min=square_x_min,
        y_min=square_y_min,
        width=square_side,
        height=square_side,
    ...
    # Save sensor data
    sensor_file = output_dir / "square_sensor_data.npz"
