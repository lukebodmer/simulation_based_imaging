#!/usr/bin/env python
"""Hyperparameter sweep for 2D CNN use_residual.

This script trains 2D CNN models with and without residual connections
and produces comparison charts for the dissertation.

Usage:
    python -m sbimaging.scripts.sweep_cnn2d_use_residual --batch <batch_name>

Example:
    python -m sbimaging.scripts.sweep_cnn2d_use_residual --batch 3d_cubes_500
"""
...
class SweepConfig:
    ...
    # Sweep parameter
    use_residual_options: list[bool] = field(
...
def train_single_config(
    ...
    residual_str = "with_residual" if use_residual else "no_residual"
    model_name = f"cnn2d_sweep_{residual_str}"

    logger.info(f"Training {model_name} with use_residual={use_residual}")
...
def generate_charts(
    ...
    # Prepare data
    labels = ["No Residual", "With Residual"]
    ...
    ax.set_xlabel("Residual Connections", fontsize=12)
    ax.set_ylabel("Voxel Error (Sum of Absolute Errors)", fontsize=12)
    ax.set_title("2D CNN: Voxel Error vs Residual Connections", fontsize=14)
    ax.set_xticks(x_pos)
...
def run_sweep(batch_name: str, config: SweepConfig) -> None:
    ...
    logger.info(f"Starting use_residual sweep for batch: {batch_name}")
    ...
    logger.info(f"Sweep options: {config.use_residual_options}")
    ...
    for use_residual in config.use_residual_options:
        result = train_single_config(
            X, y, sample_ids, num_sensors, use_residual, config, models_dir
        )
    ...
    sweep_name = "sweep_cnn2d_use_residual"
    ...
    print("=" * 80)
    print(f"{'Use Residual':<15} {'Train Loss':<12} {'Test Loss':<12} {'Voxel Error':<12}")
    print("-" * 80)
    for r in results:
        print(
            f"{str(r.use_residual):<15} {r.train_loss:<12.6e} "
    ...
    print(f"\nBest configuration: use_residual={best.use_residual}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter sweep for 2D CNN use_residual"
    )
    parser.add_argument(
        "--batch",
        required=True,
