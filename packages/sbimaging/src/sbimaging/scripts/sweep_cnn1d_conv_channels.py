#!/usr/bin/env python
"""Hyperparameter sweep for 1D CNN conv_channels.

This script trains multiple 1D CNN models with different conv_channels
configurations and produces comparison charts for the dissertation.

Usage:
    python -m sbimaging.scripts.sweep_cnn1d_conv_channels --batch <batch_name>

Example:
    python -m sbimaging.scripts.sweep_cnn1d_conv_channels --batch 3d_cubes_500
"""
...
class SweepConfig:
    ...
    # 1D CNN parameters (fixed except conv_channels)
    pool_size: int = 16
    regressor_hidden: int = 1024
    dropout: float = 0.3
    use_residual: bool = True

    # Sweep parameter
    conv_channels_options: list[list[int]] = field(
...
def train_single_config(
    ...
    channels_str = "_".join(map(str, conv_channels))
    model_name = f"cnn1d_sweep_channels_{channels_str}"

    logger.info(f"Training {model_name} with conv_channels={conv_channels}")
...
def generate_charts(
    ...
    # Prepare data
    labels = [str(r.conv_channels) for r in results]
    ...
    ax.set_xlabel("Conv Channels Configuration", fontsize=12)
    ax.set_ylabel("Voxel Error (Sum of Absolute Errors)", fontsize=12)
    ax.set_title("1D CNN: Voxel Error vs Conv Channels", fontsize=14)
    ax.set_xticks(x_pos)
...
def run_sweep(batch_name: str, config: SweepConfig) -> None:
    ...
    logger.info(f"Starting conv_channels sweep for batch: {batch_name}")
    ...
    logger.info(f"Sweep options: {config.conv_channels_options}")
    ...
    for conv_channels in config.conv_channels_options:
        result = train_single_config(
            X, y, sample_ids, num_sensors, conv_channels, config, models_dir
        )
    ...
    sweep_name = "sweep_cnn1d_conv_channels"
    ...
    print("=" * 80)
    print(f"{'Conv Channels':<25} {'Train Loss':<12} {'Test Loss':<12} {'Voxel Error':<12}")
    print("-" * 80)
    for r in results:
        print(
            f"{str(r.conv_channels):<25} {r.train_loss:<12.6e} "
    ...
    print(f"\nBest configuration: conv_channels={best.conv_channels}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter sweep for 1D CNN conv_channels"
    )
    parser.add_argument(
        "--batch",
        required=True,
