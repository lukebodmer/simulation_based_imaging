#!/usr/bin/env python
"""Hyperparameter sweep for 1D CNN dropout.

This script trains multiple 1D CNN models with different dropout
configurations and produces comparison charts for the dissertation.

Usage:
    python -m sbimaging.scripts.sweep_cnn1d_dropout --batch <batch_name>

Example:
    python -m sbimaging.scripts.sweep_cnn1d_dropout --batch 3d_cubes_500
"""
...
class SweepConfig:
    ...
    # 1D CNN parameters (fixed except dropout)
...
def train_single_config(
    ...
    model_name = f"cnn1d_sweep_dropout_{dropout:.1f}".replace(".", "_")
...
def generate_charts(
    ...
    # Prepare data
    labels = [str(r.dropout) for r in results]
    ...
    ax.set_title("1D CNN: Voxel Error vs Dropout Rate", fontsize=14)
    ax.set_xticks(x_pos)
...
def run_sweep(batch_name: str, config: SweepConfig) -> None:
    ...
    for dropout in config.dropout_options:
        result = train_single_config(
            X, y, sample_ids, num_sensors, dropout, config, models_dir
        )
    ...
    sweep_name = "sweep_cnn1d_dropout"
    ...
    for r in results:
        print(
            f"{r.dropout:<12} {r.train_loss:<12.6e} "
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter sweep for 1D CNN dropout"
    )
    parser.add_argument(
        "--batch",
        required=True,
