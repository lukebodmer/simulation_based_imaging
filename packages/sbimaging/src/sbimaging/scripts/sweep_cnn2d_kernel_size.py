#!/usr/bin/env python
"""Hyperparameter sweep for 2D CNN kernel_size.

This script trains multiple 2D CNN models with different kernel_size
configurations and produces comparison charts for the dissertation.

Usage:
    python -m sbimaging.scripts.sweep_cnn2d_kernel_size --batch <batch_name>

Example:
    python -m sbimaging.scripts.sweep_cnn2d_kernel_size --batch 3d_cubes_500
"""
...
class SweepConfig:
    ...
    # 2D CNN parameters (fixed based on previous sweeps)
    ...
    # Sweep parameter
    kernel_size_options: list[tuple[int, int]] = field(
        default_factory=lambda: [
            (1, 11),   # Minimal spatial, default temporal
            (3, 5),    # Small temporal receptive field
            (3, 7),    # Medium-small temporal
            (3, 11),   # Current default
            (3, 15),   # Larger temporal
            (5, 11),   # Larger spatial
            (7, 7),    # Square kernel
            (7, 11),   # Large spatial, default temporal
        ]
    )
...
def train_single_config(
    ...
    kernel_str = f"{kernel_size[0]}x{kernel_size[1]}"
    model_name = f"cnn2d_sweep_kernel_{kernel_str}"

    logger.info(f"Training {model_name} with kernel_size={kernel_size}")
...
def generate_charts(
    ...
    # Prepare data
    labels = [f"{r.kernel_size[0]}x{r.kernel_size[1]}" for r in results]
    ...
    ax.set_xlabel("Kernel Size (H x W)", fontsize=12)
    ax.set_ylabel("Voxel Error (Sum of Absolute Errors)", fontsize=12)
    ax.set_title("2D CNN: Voxel Error vs Kernel Size", fontsize=14)
    ax.set_xticks(x_pos)
    ...
    ax.set_title("2D CNN: Training vs Test Loss", fontsize=14)
    ax.set_xticks(x_pos)
    ax.set_xticklabels(labels, rotation=45, ha="right", fontsize=10)
    ax.legend()
    ax.set_yscale("log")
...
def run_sweep(batch_name: str, config: SweepConfig) -> None:
    ...
    logger.info(f"Starting kernel_size sweep for batch: {batch_name}")
    ...
    logger.info(f"Sweep options: {config.kernel_size_options}")
    ...
    for kernel_size in config.kernel_size_options:
        result = train_single_config(
            X, y, sample_ids, num_sensors, kernel_size, config, models_dir
        )
    ...
    sweep_name = "sweep_cnn2d_kernel_size"
    ...
    print("=" * 80)
    print(f"{'Kernel Size':<15} {'Train Loss':<12} {'Test Loss':<12} {'Voxel Error':<12}")
    print("-" * 80)
    for r in results:
        kernel_str = f"{r.kernel_size[0]}x{r.kernel_size[1]}"
        print(
            f"{kernel_str:<15} {r.train_loss:<12.6e} "
    ...
    print(f"\nBest configuration: kernel_size={best.kernel_size}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter sweep for 2D CNN kernel_size"
    )
    parser.add_argument(
        "--batch",
        required=True,
