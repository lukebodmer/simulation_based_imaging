#!/usr/bin/env python
"""Hyperparameter sweep for 1D CNN regressor_hidden.

This script trains multiple 1D CNN models with different regressor_hidden
configurations and produces comparison charts for the dissertation.

Usage:
    python -m sbimaging.scripts.sweep_cnn1d_regressor_hidden --batch <batch_name>

Example:
    python -m sbimaging.scripts.sweep_cnn1d_regressor_hidden --batch 3d_cubes_500
"""
...
class SweepConfig:
    ...
    # 1D CNN parameters (fixed except regressor_hidden)
    ...
    # Sweep parameter
    regressor_hidden_options: list[int] = field(
...
def train_single_config(
    ...
    model_name = f"cnn1d_sweep_regressor_{regressor_hidden}"

    logger.info(f"Training {model_name} with regressor_hidden={regressor_hidden}")

    model = NeuralNetworkModel(
        name=model_name,
        architecture="cnn",
        cnn_conv_channels=config.conv_channels,
        cnn_regressor_hidden=regressor_hidden,
        cnn_dropout=config.dropout,
        cnn_use_residual=config.use_residual,
        cnn_pool_size=config.pool_size,
    )
...
def generate_charts(
    ...
    # Prepare data
    labels = [str(r.regressor_hidden) for r in results]
    ...
    ax.set_xlabel("Regressor Hidden Size", fontsize=12)
    ax.set_ylabel("Voxel Error (Sum of Absolute Errors)", fontsize=12)
    ax.set_title("1D CNN: Voxel Error vs Regressor Hidden Size", fontsize=14)
    ax.set_xticks(x_pos)
    ...
    ax.set_title("1D CNN: Training vs Test Loss", fontsize=14)
    ax.set_xticks(x_pos)
    ax.set_xticklabels(labels, fontsize=10)
    ax.legend()
    ax.set_yscale("log")
...
def run_sweep(batch_name: str, config: SweepConfig) -> None:
    ...
    logger.info(f"Starting regressor_hidden sweep for batch: {batch_name}")
    ...
    logger.info(f"Sweep options: {config.regressor_hidden_options}")
    ...
    for regressor_hidden in config.regressor_hidden_options:
        result = train_single_config(
            X, y, sample_ids, num_sensors, regressor_hidden, config, models_dir
        )
    ...
    summary = {
        ...
        "sweep_parameter": "regressor_hidden",
        "fixed_config": {
            "trim_timesteps": config.trim_timesteps,
            "downsample_factor": config.downsample_factor,
            "voxel_grid_size": config.voxel_grid_size,
        ...
        "results": [
            {
                "regressor_hidden": r.regressor_hidden,
    ...
    sweep_name = "sweep_cnn1d_regressor_hidden"
    ...
    print("=" * 80)
    print(f"{'Regressor Hidden':<18} {'Train Loss':<12} {'Test Loss':<12} {'Voxel Error':<12}")
    print("-" * 80)
    for r in results:
        print(
            f"{r.regressor_hidden:<18} {r.train_loss:<12.6e} "
    ...
    print(f"\nBest configuration: regressor_hidden={best.regressor_hidden}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter sweep for 1D CNN regressor_hidden"
    )
    parser.add_argument(
        "--batch",
        required=True,
