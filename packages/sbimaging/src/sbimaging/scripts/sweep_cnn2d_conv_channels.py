#!/usr/bin/env python
"""Hyperparameter sweep for 2D CNN conv_channels.

This script trains multiple 2D CNN models with different conv_channels
...
Example:
    python -m sbimaging.scripts.sweep_cnn2d_conv_channels --batch 3d_cubes_500
"""
...
class SweepConfig:
    ...
    # 2D CNN parameters (fixed except conv_channels)
    pool_size: tuple[int, int] = (12, 16)
...
def train_single_config(
    ...
    model_name = f"cnn2d_sweep_channels_{channels_str}"
...
def generate_charts(
    ...
    ax.set_title("2D CNN: Voxel Error vs Conv Channels", fontsize=14)
    ax.set_xticks(x_pos)
...
def run_sweep(batch_name: str, config: SweepConfig) -> None:
    ...
    sweep_name = "sweep_cnn2d_conv_channels"
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter sweep for 2D CNN conv_channels"
    )
    parser.add_argument(
        "--batch",
        required=True,
