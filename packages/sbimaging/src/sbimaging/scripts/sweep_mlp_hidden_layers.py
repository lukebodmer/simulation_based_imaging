#!/usr/bin/env python
"""Hyperparameter sweep for MLP hidden_layers.

This script trains multiple MLP models with different hidden_layers
configurations and produces comparison charts for the dissertation.

Usage:
    python -m sbimaging.scripts.sweep_mlp_hidden_layers --batch <batch_name>

Example:
    python -m sbimaging.scripts.sweep_mlp_hidden_layers --batch 3d_cubes_500
"""
...
class SweepConfig:
    ...
    # MLP parameters (fixed except hidden_layers)
    dropout: float = 0.2

    # Sweep parameter
    hidden_layers_options: list[list[int]] = field(
...
def train_single_config(
    ...
    model_name = f"mlp_sweep_layers_{layers_str}"

    logger.info(f"Training {model_name} with hidden_layers={hidden_layers}")
...
def generate_charts(
    ...
    ax.set_xlabel("Hidden Layers Configuration", fontsize=12)
    ax.set_ylabel("Voxel Error (Sum of Absolute Errors)", fontsize=12)
    ax.set_title("MLP: Voxel Error vs Hidden Layers", fontsize=14)
    ax.set_xticks(x_pos)
...
def run_sweep(batch_name: str, config: SweepConfig) -> None:
    ...
    logger.info(f"Starting hidden_layers sweep for batch: {batch_name}")
    ...
    logger.info(f"Sweep options: {config.hidden_layers_options}")
    ...
    for hidden_layers in config.hidden_layers_options:
        result = train_single_config(
            X, y, sample_ids, hidden_layers, config, models_dir
        )
    ...
    sweep_name = "sweep_mlp_hidden_layers"
    ...
    print("=" * 90)
    print(f"{'Hidden Layers':<35} {'Train Loss':<12} {'Test Loss':<12} {'Voxel Error':<12}")
    print("-" * 90)
    for r in results:
        print(
            f"{str(r.hidden_layers):<35} {r.train_loss:<12.6e} "
    ...
    print(f"\nBest configuration: hidden_layers={best.hidden_layers}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter sweep for MLP hidden_layers"
    )
    parser.add_argument(
        "--batch",
        required=True,
