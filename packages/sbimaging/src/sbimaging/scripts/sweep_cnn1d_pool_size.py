#!/usr/bin/env python
"""Hyperparameter sweep for 1D CNN pool_size.

This script trains multiple 1D CNN models with different pool_size
configurations and produces comparison charts for the dissertation.

Usage:
    python -m sbimaging.scripts.sweep_cnn1d_pool_size --batch <batch_name>

Example:
    python -m sbimaging.scripts.sweep_cnn1d_pool_size --batch 3d_cubes_500
"""
...
class SweepConfig:
    ...
    # 1D CNN parameters (fixed except pool_size)
    ...
    # Sweep parameter
    pool_size_options: list[int] = field(
        default_factory=lambda: [8, 16, 24, 32, 48]
    )
...
def train_single_config(
    ...
    model_name = f"cnn1d_sweep_pool_{pool_size}"
...
def generate_charts(
    ...
    # Prepare data
    labels = [str(r.pool_size) for r in results]
    ...
    ax.set_xlabel("Pool Size", fontsize=12)
    ax.set_ylabel("Voxel Error (Sum of Absolute Errors)", fontsize=12)
    ax.set_title("1D CNN: Voxel Error vs Pool Size", fontsize=14)
    ax.set_xticks(x_pos)
...
def run_sweep(batch_name: str, config: SweepConfig) -> None:
    ...
    sweep_name = "sweep_cnn1d_pool_size"
    ...
    print("=" * 80)
    print(f"{'Pool Size':<12} {'Train Loss':<12} {'Test Loss':<12} {'Voxel Error':<12}")
    print("-" * 80)
    for r in results:
        print(
            f"{r.pool_size:<12} {r.train_loss:<12.6e} "
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter sweep for 1D CNN pool_size"
    )
    parser.add_argument(
        "--batch",
        required=True,
