#!/usr/bin/env python
"""Hyperparameter sweep for 2D CNN regressor_hidden.

This script trains multiple 2D CNN models with different regressor_hidden
configurations and produces comparison charts for the dissertation.

Usage:
    python -m sbimaging.scripts.sweep_cnn2d_regressor_hidden --batch <batch_name>

Example:
    python -m sbimaging.scripts.sweep_cnn2d_regressor_hidden --batch 3d_cubes_500
"""
...
def train_single_config(
    ...
    model_name = f"cnn2d_sweep_regressor_{regressor_hidden}"
...
def generate_charts(
    ...
    ax.set_title("2D CNN: Voxel Error vs Regressor Hidden Size", fontsize=14)
    ax.set_xticks(x_pos)
...
def run_sweep(batch_name: str, config: SweepConfig) -> None:
    ...
    sweep_name = "sweep_cnn2d_regressor_hidden"
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter sweep for 2D CNN regressor_hidden"
    )
    parser.add_argument(
        "--batch",
        required=True,
