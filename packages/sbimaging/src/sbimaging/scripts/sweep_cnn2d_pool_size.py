#!/usr/bin/env python
"""Hyperparameter sweep for 2D CNN pool_size.

This script trains multiple 2D CNN models with different pool_size
configurations and produces comparison charts for the dissertation.

Usage:
    python -m sbimaging.scripts.sweep_cnn2d_pool_size --batch <batch_name>

Example:
    python -m sbimaging.scripts.sweep_cnn2d_pool_size --batch 3d_cubes_500
"""
...
class SweepConfig:
    ...
    # 2D CNN parameters (fixed based on previous sweep)
    ...
    # Sweep parameter
    pool_size_options: list[tuple[int, int]] = field(
...
def train_single_config(
    ...
    pool_str = f"{pool_size[0]}x{pool_size[1]}"
    model_name = f"cnn2d_sweep_pool_{pool_str}"

    logger.info(f"Training {model_name} with pool_size={pool_size}")
...
def generate_charts(
    ...
    # Prepare data
    labels = [f"{r.pool_size[0]}x{r.pool_size[1]}" for r in results]
    ...
    ax.set_xlabel("Pool Size (H x W)", fontsize=12)
    ax.set_ylabel("Voxel Error (Sum of Absolute Errors)", fontsize=12)
    ax.set_title("2D CNN: Voxel Error vs Pool Size", fontsize=14)
    ax.set_xticks(x_pos)
...
def run_sweep(batch_name: str, config: SweepConfig) -> None:
    ...
    logger.info(f"Starting pool_size sweep for batch: {batch_name}")
    ...
    logger.info(f"Sweep options: {config.pool_size_options}")
    ...
    for pool_size in config.pool_size_options:
        result = train_single_config(
            X, y, sample_ids, num_sensors, pool_size, config, models_dir
        )
    ...
    sweep_name = "sweep_cnn2d_pool_size"
    ...
    print("=" * 80)
    print(f"{'Pool Size':<15} {'Train Loss':<12} {'Test Loss':<12} {'Voxel Error':<12}")
    print("-" * 80)
    for r in results:
        pool_str = f"{r.pool_size[0]}x{r.pool_size[1]}"
        print(
            f"{pool_str:<15} {r.train_loss:<12.6e} "
    ...
    print(f"\nBest configuration: pool_size={best.pool_size}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter sweep for 2D CNN pool_size"
    )
    parser.add_argument(
        "--batch",
        required=True,
