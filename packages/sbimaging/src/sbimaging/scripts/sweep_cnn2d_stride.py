#!/usr/bin/env python
"""Hyperparameter sweep for 2D CNN stride.

This script trains multiple 2D CNN models with different stride
configurations and produces comparison charts for the dissertation.

Usage:
    python -m sbimaging.scripts.sweep_cnn2d_stride --batch <batch_name>

Example:
    python -m sbimaging.scripts.sweep_cnn2d_stride --batch 3d_cubes_500
"""
...
class SweepConfig:
    ...
    # Sweep parameter
    stride_options: list[tuple[int, int]] = field(
        default_factory=lambda: [
            (1, 1),    # No downsampling
            (1, 2),    # Current default (preserve spatial, downsample time)
            (2, 2),    # Uniform downsampling
            (1, 3),    # Aggressive time downsampling
        ]
    )
...
def train_single_config(
    ...
    stride_str = f"{stride[0]}x{stride[1]}"
    model_name = f"cnn2d_sweep_stride_{stride_str}"

    logger.info(f"Training {model_name} with stride={stride}")
...
def generate_charts(
    ...
    # Prepare data
    labels = [f"{r.stride[0]}x{r.stride[1]}" for r in results]
    ...
    ax.set_xlabel("Stride (H x W)", fontsize=12)
    ax.set_ylabel("Voxel Error (Sum of Absolute Errors)", fontsize=12)
    ax.set_title("2D CNN: Voxel Error vs Stride", fontsize=14)
    ax.set_xticks(x_pos)
...
def run_sweep(batch_name: str, config: SweepConfig) -> None:
    ...
    logger.info(f"Starting stride sweep for batch: {batch_name}")
    ...
    logger.info(f"Sweep options: {config.stride_options}")
    ...
    for stride in config.stride_options:
        result = train_single_config(
            X, y, sample_ids, num_sensors, stride, config, models_dir
        )
    ...
    print("=" * 80)
    print(f"{'Stride':<12} {'Train Loss':<12} {'Test Loss':<12} {'Voxel Error':<12}")
    print("-" * 80)
    for r in results:
        stride_str = f"{r.stride[0]}x{r.stride[1]}"
        print(
            f"{stride_str:<12} {r.train_loss:<12.6e} "
    ...
    print(f"\nBest configuration: stride={best.stride}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter sweep for 2D CNN stride"
    )
    parser.add_argument(
        "--batch",
        required=True,
